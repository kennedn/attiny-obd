...
const char storage_stored_prefix_2[] PROGMEM = "  Last: ";
...
const char storage_icon_4[] PROGMEM = {0x1c, 0x08, 0x1d, 0x1f, 0x1f, 0x1d, 0x08, 0x1c};    // engine

void storage_write_long(unsigned char slot, long data)
{
    eeprom_write_dword((uint32_t*)(slot*4), (uint32_t)data);
}

long storage_read_long(unsigned char slot)
{
    return eeprom_read_dword((uint32_t*)(slot*4));
}
